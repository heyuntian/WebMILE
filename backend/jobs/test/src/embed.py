...
if __name__ == '__main__':
    args = parse_args()
    dw_args, (graph, _) = DeepWalkSetting(), read_graph(None, args.input,
                                                   edgelist=(args.input[-8:] == 'edgelist'),
                                                   metis=(args.input[-5:] == 'metis'))
    embeddings = DeepWalk_Original(dw_args, embed_dim=args.embed_dim, workers=args.workers, graph=graph).get_embeddings()
